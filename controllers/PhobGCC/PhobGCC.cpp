...
uint16_t get_buttons() {
    ...
    return (gpio_get(DPAD_LEFT_PIN) << DPAD_LEFT) | (gpio_get(DPAD_RIGHT_PIN) << DPAD_RIGHT) |
        (gpio_get(DPAD_DOWN_PIN) << DPAD_DOWN) | (gpio_get(DPAD_UP_PIN) << DPAD_UP) |
        (gpio_get(Z_PIN) << Z) | (gpio_get(RT_DIGITAL_PIN) << RT_DIGITAL) |
...
void read_mcp3202_data(uint cs_pin, bool read_y_axis, uint16_t &out) {
    ...
    out = (((stick_raw[0] & 0b00000111) << 9) | stick_raw[1] << 1 | stick_raw[2] >> 7);
