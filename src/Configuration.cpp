...
controller_config load_config() {
    ...
    return *reinterpret_cast<controller_config*>(CONFIG_SRAM_BASE + (read_page * FLASH_PAGE_SIZE));
} 
...
void persist_config(controller_config *config) {
    ...
    flash_range_program(CONFIG_FLASH_BASE + (write_page * FLASH_PAGE_SIZE), buf, FLASH_PAGE_SIZE);
}